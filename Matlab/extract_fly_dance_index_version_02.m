function [output] = extract_fly_dance_index_version_02(file_no, coordinate_file_name)
%   Extract Fly Dance Index for each data point

% -------------------------------------------------------
    bin_duration = 10;  % sec
    camera_frame_rate = 40; % fps
% -------------------------------------------------------
...
function [output] = calculate_locomotive_time(data, bin_duration, camera_frame_rate)
    output = 0;
    no_frames_per_sec = floor(length(data.x) / bin_duration);
    start_frame = 1;
    
    for i = 1:bin_duration
        end_frame = i * no_frames_per_sec;
        
        x = data.x(start_frame:end_frame, 1);
        y = data.y(start_frame:end_frame, 1);
        ...
        if (sum_distance > 2)
            output = output + no_frames_per_sec;
        end
        start_frame = end_frame + 1;
    end
    output = (output / camera_frame_rate);
end